...
void oledPolyline(const OLED_VERTEX *par_vertex, uint16_t par_size, OLED_COLOR color) {
  ...
  if(par_vertex != 0){
    for(i = 1; i < par_size; i++){
      oledLine(par_vertex[i - 1].x, par_vertex[i - 1].y, par_vertex[i].x, par_vertex[i].y, color);
    }
  }
  else
  {
...
static float oledDegToRad(float par_deg) {
...
void oledDrawArc(uint8_t x, uint8_t y, uint8_t radius, uint16_t start_angle, uint16_t sweep, OLED_COLOR color) {
    ...
    count = (oledNormalizeTo0_360(start_angle) * CIRCLE_APPROXIMATION_SEGMENTS) / 360;
    ...
    while(count < approx_segments)
    {
        ...
        oledLine(xp1,yp1,xp2,yp2,color);
    }
...
void oledDrawRectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, OLED_COLOR color) {
  oledLine(x1,y1,x2,y1,color);
  oledLine(x2,y1,x2,y2,color);
  oledLine(x2,y2,x1,y2,color);
  oledLine(x1,y2,x1,y1,color);
